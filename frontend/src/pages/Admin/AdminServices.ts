export const adminLogin = async (
  ...
  export const getMarketStatus = async (): Promise<string> => {
    ...
    // Extract the actual status from the returned object
    return data.data;
  };
  ...
  export const getPriceUpdatesAlgorithm = async (): Promise<any> => {
    const response = await fetch('/api/v1/market/price-updates/algorithm', {
      method: 'GET',
      credentials: 'include',
    });
    const data = await response.json();
    if (!response.ok)
      throw new Error(data.message || 'Failed to fetch algorithm price updates');
    return data.data;
  };
  ...
  export const manualPriceUpdate = async (
    ...
    const response = await fetch('/api/v1/market/price-updates/manual', {
      method: 'POST',
      credentials: 'include',
      headers: { 'Content-Type': 'application/json' },
